...
function Testimonials() {
  ...
  const firstThree = visibleTestimonials.slice(0, 3);
  const rest = visibleTestimonials.slice(3);
  ...
  return (
    ...
    >
      <div className="relative z-20 w-full max-w-7xl mx-auto">
        ...
        {/* Baris 1: testi1 (col-span-2) + testi2 (col-span-1) */}
        {firstThree.length >= 2 && (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 auto-rows-fr items-stretch mb-6">
            <div className="md:col-span-2">
              <TestimonialCard testimonial={firstThree[0]} index={0} />
            </div>
            <div>
              <TestimonialCard testimonial={firstThree[1]} index={1} />
            </div>
          </div>
        )}

        {/* Baris 2: testi3 full width (col-span-3) tanpa row-span */}
        {firstThree.length >= 3 && (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
            <div className="md:col-span-3">
              <TestimonialCard testimonial={firstThree[2]} index={2} />
            </div>
          </div>
        )}

        {/* Grid normal untuk testimonial sisanya */}
        {rest.length > 0 && (
