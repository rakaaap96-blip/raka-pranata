...
const waveBarHeights = Array.from({ length: 12 }, () => `${20 + Math.random() * 80}%`);
...
function ProjectFilter({ filter, setFilter, searchTerm, setSearchTerm }: ProjectFilterProps) {
  ...
  return (
    <div className="relative mb-8 sm:mb-12 lg:mb-16">
      {/* Main Container */}
      <div className="relative p-4 sm:p-6 lg:p-8 cyber-panel cyber-frame overflow-hidden">
        ...
        <div className="relative z-10 flex flex-col lg:flex-row gap-4 sm:gap-6 items-stretch lg:items-center justify-between">
          ...
          <div className="relative w-full lg:w-80 lg:shrink-0 group">
            ...
            <div
              className={`absolute -inset-px cyber-button-cut bg-linear-to-r from-[#d4af37] via-[#f4d03f] to-[#d4af37] opacity-0 blur-sm transition-opacity duration-500 bg-[size:200%_100%] ${
                isFocused ? 'opacity-60 animate-spin-slow' : ''
              }`}
            />

            <div className="relative bg-[#050505] cyber-button-cut border border-[#d4af37]/30 overflow-hidden">
              {/* Typing Wave */}
              <div
          ...
          {/* Filter Buttons */}
          <div className="relative flex flex-wrap gap-1.5 sm:gap-2 justify-center items-center p-1 sm:p-1.5 cyber-chip">
